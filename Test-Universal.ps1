...
try {
    ...
    Write-Host "    Debug: TokenInfo Type: $($tokenInfo.GetType().FullName)" -ForegroundColor Gray
    if ($tokenInfo.Token) {
        Write-Host "    Debug: Token Property Type: $($tokenInfo.Token.GetType().FullName)" -ForegroundColor Gray
        Write-Host "    Debug: Token Property Length: $($tokenInfo.Token.Length)" -ForegroundColor Gray
    }
    ...
    if ($tokenInfo.Token -is [System.Security.SecureString]) {
        # Windows PowerShell 5.1 with older Az modules
        Write-Host "    Debug: Converting from SecureString" -ForegroundColor Gray
        $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($tokenInfo.Token)
        $token = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)
        [System.Runtime.InteropServices.Marshal]::ZeroFreeBSTR($BSTR)
    } elseif ($tokenInfo.Token -and $tokenInfo.Token -is [string]) {
        # Modern Az modules or PowerShell 7+ with string token
        Write-Host "    Debug: Using Token property (string)" -ForegroundColor Gray
        $token = $tokenInfo.Token
    } elseif ($tokenInfo -is [string]) {
        # Direct string return (some environments)
        Write-Host "    Debug: Using tokenInfo directly (string)" -ForegroundColor Gray
        $token = $tokenInfo
    } elseif ($tokenInfo.Token) {
        # Try to convert to string
        Write-Host "    Debug: Converting Token to string" -ForegroundColor Gray
        $token = $tokenInfo.Token.ToString()
    } else {
        Write-Host "  [X] Unknown token format: $($tokenInfo.GetType().Name)" -ForegroundColor Red
        Write-Host "    Properties: $($tokenInfo | Get-Member -MemberType Property | ForEach-Object { $_.Name } | Join-String -Separator ', ')" -ForegroundColor Gray
        Write-Host ""
        return
    }
