...
if (-not [string]::IsNullOrWhiteSpace($SENTINEL_WORKSPACE_ID)) {
    ...
    Write-Host ""
    Write-Host "az role assignment create " -ForegroundColor Cyan -NoNewline
    Write-Host "`"
    Write-Host "  --assignee <SERVICE_PRINCIPAL_ID> " -ForegroundColor Cyan -NoNewline
    Write-Host "`"
    Write-Host "  --role 'Log Analytics Reader' " -ForegroundColor Cyan -NoNewline
    Write-Host "`"
    Write-Host "  --scope <LOG_ANALYTICS_WORKSPACE_RESOURCE_ID>" -ForegroundColor Cyan
}
Write-Host ""
