...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  [!] Could not set Application ID URI" -ForegroundColor Yellow
    }
}
catch {
    Write-Host "  [!] Could not set Application ID URI" -ForegroundColor Yellow
...
# Get the app's object ID for Graph API call
try {
    $APP_OBJECT_ID = az ad app show --id $APP_ID --query "id" -o tsv
    
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($APP_OBJECT_ID)) {
        throw "Failed to retrieve app object ID"
    }

    # Add user_impersonation scope via Graph API
    ...
    # Create temp file for JSON body (Cloud Shell compatible)
    $tempDir = if ($env:TEMP) { $env:TEMP } else { "/tmp" }
    $scopeJsonPath = Join-Path $tempDir "dun-api-scope-$([guid]::NewGuid().ToString()).json"
    $scopeBody | ConvertTo-Json -Depth 10 | Out-File -FilePath $scopeJsonPath -Encoding utf8 -NoNewline

    # Update app with API scope
    $scopeResult = az rest --method PATCH --uri "https://graph.microsoft.com/v1.0/applications/$APP_OBJECT_ID" --headers "Content-Type=application/json" --body "@$scopeJsonPath" 2>&1
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] API scope configured for OBO token exchange" -ForegroundColor Green
    } else {
        Write-Host "  [!] Could not configure API scope (may require manual setup)" -ForegroundColor Yellow
    }

    # Clean up temp file
    Remove-Item -Path $scopeJsonPath -ErrorAction SilentlyContinue
}
catch {
    Write-Host "  [!] Could not configure API scope (may require manual setup)" -ForegroundColor Yellow
