...
# List all subscriptions for user confirmation
$SUB_INDEX = 1
foreach ($subscription in $SUBSCRIPTIONS) {
    Write-Host "  " -NoNewline
    Write-Host "$SUB_INDEX." -ForegroundColor Cyan -NoNewline
    Write-Host " $($subscription.name)"
    Write-Host "     ($($subscription.id))" -ForegroundColor DarkGray
    $SUB_INDEX++
}

Write-Host ""
Write-Host "[!] DUN Security will have READ-ONLY access to ALL these subscriptions." -ForegroundColor Yellow
Write-Host ""
