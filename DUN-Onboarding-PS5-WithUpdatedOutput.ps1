...
# ============================================================================
# Choose Primary Subscription
# ============================================================================
Write-Host "Discovering subscriptions..." -ForegroundColor Yellow
Write-Host ""

# Get only ENABLED subscriptions
$ALL_SUBSCRIPTIONS = az account list --query "[?state=='Enabled'].{id:id, name:name}" -o json | ConvertFrom-Json
$ALL_SUB_COUNT = $ALL_SUBSCRIPTIONS.Count

if ($ALL_SUB_COUNT -eq 0) {
...
Write-Host $ALL_SUB_COUNT -ForegroundColor Cyan -NoNewline
Write-Host " subscription(s). Please choose your primary subscription:" -ForegroundColor Yellow
Write-Host "================================================================" -ForegroundColor Yellow
Write-Host ""

# List all subscriptions - display each on its own line
foreach ($subscription in $ALL_SUBSCRIPTIONS) {
    $index = $ALL_SUBSCRIPTIONS.IndexOf($subscription) + 1
    $cleanName = Clean-DisplayText $subscription.name
    Write-Host "  $index. $cleanName" -ForegroundColor Cyan
}

Write-Host ""
Write-Host "Enter the number of your primary subscription:" -ForegroundColor Yellow
Write-Host ""
$PRIMARY_SELECTION = Read-Host ">"

# Validate selection
if ([string]::IsNullOrWhiteSpace($PRIMARY_SELECTION)) {
    Write-Host ""
...
try {
    $PRIMARY_INDEX = [int]$PRIMARY_SELECTION
    
    if ($PRIMARY_INDEX -lt 1 -or $PRIMARY_INDEX -gt $ALL_SUB_COUNT) {
        Write-Host ""
        Write-Host "[X] Invalid selection. Please choose a number between 1 and $ALL_SUB_COUNT" -ForegroundColor Red
        exit 1
    }
    
    $PRIMARY_SUBSCRIPTION = $ALL_SUBSCRIPTIONS[$PRIMARY_INDEX - 1]
    $PRIMARY_SUB_NAME = Clean-DisplayText $PRIMARY_SUBSCRIPTION.name
    
    # Set as default subscription
    az account set --subscription $PRIMARY_SUBSCRIPTION.id 2>$null
    
    Write-Host ""
    Write-Host "[OK] Primary subscription set: " -ForegroundColor Green -NoNewline
    Write-Host $PRIMARY_SUB_NAME -ForegroundColor Cyan
    Write-Host "     ID: " -NoNewline
    Write-Host $PRIMARY_SUBSCRIPTION.id -ForegroundColor Cyan
    Write-Host ""
    
} catch {
    Write-Host ""
    Write-Host "[X] Invalid input. Please enter a valid number." -ForegroundColor Red
    exit 1
}

# Display tenant info after subscription selection
