...
if ($ENABLE_FLOW_LOGS) {
    ...
    Write-Host "  This requires a Log Analytics Workspace and has additional cost (~`$2.50/GB)."
Write-Host ""
        ...
        foreach ($REGION in $REGIONS) {
            ...
            if (-not $StorageExists -or $LASTEXITCODE -ne 0) {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    $FlowLogStats.Errors += "Failed to create storage account $STORAGE_NAME: $createResult"
...
# - Complete Flow Logs configuration implementation
