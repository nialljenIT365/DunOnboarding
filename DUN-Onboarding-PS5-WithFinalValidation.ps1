...
# Get the actual tenant name (not subscription name)
try {
    ...
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($tenantInfo)) {
        ...
        # Fallback to tenant ID if we can't get the name
...
if (-not $APP_OBJECT_ID) {
    Write-Host "  [!] App objectId is not available, client secret creation will be skipped" -ForegroundColor Yellow
    Write-Host "  You can create a secret manually in Azure Portal:" -ForegroundColor Yellow
    Write-Host "  https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/Credentials/appId/$APP_ID" -ForegroundColor Blue
} else {
    ...
    try {
        ...
        if ($CLIENT_SECRET) {
            Write-Host "  [OK] Client secret created (expires: $endDateTime)" -ForegroundColor Green
            Write-Host ""
            ...
            Write-Host "  [!] Graph call succeeded but secretText was empty (unexpected)" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "  [!] Failed to create client secret via Graph" -ForegroundColor Yellow
        Write-Host "  Error: $_" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "  You can create a secret manually in Azure Portal:" -ForegroundColor Yellow
        Write-Host "  https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/Credentials/appId=$APP_ID" -ForegroundColor Blue
...
$consentResult = az ad app permission admin-consent --id $APP_ID --only-show-errors 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Admin consent granted" -ForegroundColor Green
} else {
    Write-Host "[!] First attempt failed. Waiting 5 seconds and retrying..." -ForegroundColor Yellow
    Start-Sleep -Seconds 5
    
    Write-Host "Retrying admin consent..."
    $consentResult = az ad app permission admin-consent --id $APP_ID --only-show-errors 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Admin consent granted on retry" -ForegroundColor Green
    } else {
...
Write-Host ""
Write-Host "Enable Virtual CSOC integration? [Y/N] (default: No)" -ForegroundColor Yellow
$SENTINEL_INPUT = Read-Host ">"

# Default to No if empty or not Y
$ENABLE_SENTINEL = $SENTINEL_INPUT -match '^[Yy]$'

if ($ENABLE_SENTINEL) {
    Write-Host ""
    Write-Host "Enter your Sentinel Log Analytics Workspace ID:" -ForegroundColor Yellow
    Write-Host "(Find in: Azure Portal -> Log Analytics workspaces -> Properties -> Workspace ID)" -ForegroundColor DarkGray
    Write-Host "(Leave empty to skip)" -ForegroundColor DarkGray
    Write-Host ""
    ...
    if ($WORKSPACE_FOUND) {
        ...
        Write-Host ""
        Write-Host "[!] No workspace ID provided. Virtual CSOC will be skipped." -ForegroundColor Yellow
        $SENTINEL_WORKSPACE_ID = $null
    }
} else {
    Write-Host ""
    Write-Host "[!] Virtual CSOC skipped. You can enable it later by granting access:" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  az role assignment create --assignee $SP_ID --role 'Log Analytics Reader' --scope <YOUR_WORKSPACE_RESOURCE_ID>" -ForegroundColor Cyan
    $SENTINEL_WORKSPACE_ID = $null
}
Write-Host ""
...
Write-Host "================================================================" -ForegroundColor Blue
Write-Host ""
Write-Host "Verifying all configurations..." -ForegroundColor DarkGray
Write-Host ""

$ValidationIssues = @()

# 1. Validate App Registration exists
Write-Host "[1/5] App Registration..." -NoNewline
...
if ($AppCheck) {
    ...
    $ValidationIssues += @{
        ...
        Fix = "Re-run the onboarding script or create manually in Azure Portal"
    }
}

# 2. Validate Service Principal exists
Write-Host "[2/5] Service Principal..." -NoNewline
...
if ($SpCheck) {
    ...
    $ValidationIssues += @{
        ...
        Fix = "Run: az ad sp create --id $APP_ID"
    }
}

# 3. Validate Graph API permissions with consent
Write-Host "[3/5] Graph API Permissions..." -NoNewline
$RequiredPermCount = ($GRAPH_PERMISSIONS | Where-Object { $_.Required -eq $true }).Count
$ConsentedCount = 0

try {
    # Get app Service Principal ID and check assignments via Graph API
    $appSpId = az ad sp show --id $APP_ID --query id -o tsv 2>$null
    if ($appSpId) {
        $token = az account get-access-token --resource https://graph.microsoft.com --query accessToken -o tsv 2>$null
        if ($token) {
            $headers = @{ "Authorization" = "Bearer $token" }
            $assignments = Invoke-RestMethod -Uri "https://graph.microsoft.com/v1.0/servicePrincipals/$appSpId/appRoleAssignments" -Headers $headers -ErrorAction Stop
            $ConsentedCount = if ($assignments.value) { $assignments.value.Count } else { 0 }
        }
    }
} catch {
    # If Graph API check fails, fall back to checking via az cli
    $ConsentedPerms = az ad sp show --id $APP_ID --query "appRoleAssignments[].appRoleId" -o json 2>$null | ConvertFrom-Json
    $ConsentedCount = if ($ConsentedPerms) { $ConsentedPerms.Count } else { 0 }
}

if ($ConsentedCount -ge $RequiredPermCount) {
    Write-Host " [OK] ($ConsentedCount permissions consented)" -ForegroundColor Green
} elseif ($ConsentedCount -gt 0) {
    Write-Host " [!] ($ConsentedCount of $RequiredPermCount)" -ForegroundColor Yellow
    $ValidationIssues += @{
        Issue = "Only $ConsentedCount of $RequiredPermCount Graph permissions have admin consent"
        Fix = "Grant admin consent: https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/CallAnAPI/appId=$APP_ID"
    }
} else {
    Write-Host " [X] (no consent granted)" -ForegroundColor Red
    $ValidationIssues += @{
        Issue = "Graph API permissions have NOT been admin-consented"
        Fix = "A Global Administrator must grant consent: https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/CallAnAPI/appId=$APP_ID"
    }
}

# 4. Validate RBAC roles on all selected subscriptions
Write-Host "[4/5] Azure RBAC Roles..." -NoNewline
$RbacCount = 0
$RbacFailedSubs = @()
...
if ($SUBSCRIPTIONS -and $SUBSCRIPTIONS.Count -gt 0) {
    foreach ($sub in $SUBSCRIPTIONS) {
        ...
        $ReaderAssignment = az role assignment list --assignee $SP_ID --role "Reader" --scope "/subscriptions/$($sub.id)" --query "[].id" -o tsv 2>$null
        if ($ReaderAssignment) {
            $RbacCount++
        } else {
            $RbacFailedSubs += (Clean-DisplayText $sub.name)
        }
    }
}

if ($RbacCount -eq $SUBSCRIPTIONS.Count) {
    Write-Host " [OK] (verified on all $RbacCount subscription(s))" -ForegroundColor Green
} elseif ($RbacCount -gt 0) {
    Write-Host " [!] (verified on $RbacCount of $($SUBSCRIPTIONS.Count))" -ForegroundColor Yellow
    $ValidationIssues += @{
        Issue = "RBAC roles not found on some subscriptions: $($RbacFailedSubs -join ', ')"
        Fix = "Ensure Owner or User Access Administrator role on these subscriptions, then run: az role assignment create --assignee $SP_ID --role Reader --scope /subscriptions/<SUB_ID>"
    }
} else {
    Write-Host " [X] (could not verify on any subscription)" -ForegroundColor Red
    $ValidationIssues += @{
        Issue = "Could not verify RBAC role assignments on any subscription"
        Fix = "Ensure the user running this script has Owner or User Access Administrator on subscriptions, or assign roles manually"
    }
}

# 5. Client Secret
Write-Host "[5/5] Client Secret..." -NoNewline
if ($CLIENT_SECRET) {
    Write-Host " [OK]" -ForegroundColor Green
} else {
...
if ($ValidationIssues.Count -gt 0) {
    Write-Host "================================================================" -ForegroundColor Yellow
    Write-Host "[!] $($ValidationIssues.Count) issue(s) require attention" -ForegroundColor Yellow
    Write-Host "================================================================" -ForegroundColor Yellow
    Write-Host ""
    
    $IssueNum = 1
    foreach ($issue in $ValidationIssues) {
        Write-Host "Issue $IssueNum : " -NoNewline -ForegroundColor Red
        Write-Host $issue.Issue
        Write-Host "Fix     : " -NoNewline -ForegroundColor Green
        Write-Host $issue.Fix
        Write-Host ""
        $IssueNum++
    }
} else {
...
# Note: Dashboard auto-provisioning variable ($DashboardUrl) is not implemented in this script
# If/when implemented, conditional logic can be added here

Write-Host "[OK] In order to complete onboarding to your Dashboard you will need to provide the configuration summary displayed below to our team" -ForegroundColor Green
Write-Host "[!] Do NOT paste Client Secret directly into email or chat. Send it securely as suggested below." -ForegroundColor Yellow
Write-Host ""

Write-Host "Configuration Summary (needed by DUN support):" -ForegroundColor Cyan
...
# Handle subscription display based on count
$subList = @()
if ($SUBSCRIPTIONS) {
    $subList = @($SUBSCRIPTIONS | ForEach-Object {
        [PSCustomObject]@{
            Name = Clean-DisplayText $_.name
            Id   = $_.id
        }
    })
}
...
if (-not $subList -or $subList.Count -eq 0) {
    Write-Host "[!] Subscription IDs:       (none discovered in this session)" -ForegroundColor Yellow
} elseif ($subList.Count -eq 1) {
    Write-Host "Subscription ID:        " -NoNewline
    Write-Host "$($subList[0].Id)" -ForegroundColor Cyan -NoNewline
    Write-Host " ($($subList[0].Name))"
} else {
    # If 5 or fewer subs, show compact CSV; otherwise show count + list
    $ids = @($subList | Select-Object -ExpandProperty Id)
    
    if ($ids.Count -le 5) {
        Write-Host "Subscription IDs:       " -NoNewline
        Write-Host ($ids -join ", ") -ForegroundColor Cyan
    } else {
        Write-Host "Subscription IDs:       " -NoNewline
        Write-Host "$($ids.Count)" -ForegroundColor Cyan -NoNewline
        Write-Host " (showing list below)"
    }
    
    Write-Host "Subscriptions:"
    foreach ($s in $subList) {
        Write-Host "  - $($s.Id) ($($s.Name))" -ForegroundColor Cyan
    }
}

if (-not [string]::IsNullOrWhiteSpace($SENTINEL_WORKSPACE_ID)) {
    Write-Host "Sentinel Workspace ID:  " -NoNewline
    Write-Host $SENTINEL_WORKSPACE_ID -ForegroundColor Cyan
}

Write-Host ""
Write-Host "================================================================" -ForegroundColor Yellow
Write-Host "SECURE SHARING INSTRUCTIONS (recommended)" -ForegroundColor Yellow
Write-Host "Share via OneDrive / SharePoint with an expiring link" -ForegroundColor Yellow
Write-Host "================================================================" -ForegroundColor Yellow
Write-Host ""
Write-Host "1) Save Configuration Summary to a text file (example: dun-credentials.txt)"
Write-Host "   Must include: Organization Name, Tenant Name, Tenant ID, Client ID (appId), Client Secret and Subscription ID(s)."
Write-Host ""
Write-Host "2) Upload the file to OneDrive or a SharePoint document library."
Write-Host ""
Write-Host "3) Create a sharing link:"
Write-Host "   - Right-click the file -> Share"
Write-Host "   - Click 'Link settings' and set:"
Write-Host "       * Expiration: 24-48 hours"
Write-Host "       * Optional: Require password (recommended)"
Write-Host ""
Write-Host "4) Email ONLY the sharing link (and password if used) to: " -NoNewline
Write-Host "hi@dunsecurity.ai" -ForegroundColor Cyan
Write-Host ""
Write-Host "5) After confirmation of receipt:"
Write-Host "   - Delete the file"
Write-Host "   - Revoke the sharing link / stop sharing"
Write-Host "   DUN will store the credentials securely in your dedicated Key Vault."
Write-Host ""
Write-Host "Why this is secure:" -ForegroundColor Green
Write-Host " - The secret is not sent in email."
Write-Host " - The link can expire automatically and you control access."
Write-Host " - You can revoke access at any time."
Write-Host ""
Write-Host "Any questions or concerns please contact " -NoNewline
Write-Host "hi@dunsecurity.ai" -ForegroundColor Cyan
Write-Host ""
